...
export function QuantumVisualization({ coherence }: QuantumVisualizationProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach((particle, i) => {
        ...
        particle.x += particle.vx + Math.sin(particle.phase + Date.now() * 0.001) * 0.5
        particle.y += particle.vy + Math.cos(particle.phase + Date.now() * 0.001) * 0.5
  ...
  return (
    <div className="relative h-64 w-full bg-black/20 rounded-lg overflow-hidden">
      ...
      <div className="absolute top-4 left-4 text-sm text-cyan-400">Quantum Coherence: {coherence.toFixed(1)}%</div>
    </div>
  )
}
